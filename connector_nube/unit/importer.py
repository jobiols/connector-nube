...
class TiendaNubeImporter(Importer):
    """ Base importer for TiendaNube """
    ...
    def __init__(self, environment):
        ...
        """
        super(TiendaNubeImporter, self).__init__(environment)
        self.tienda_nube_id = None
        self.tienda_nube_record = None

    def _get_tienda_nube_data(self):
        """ Return the raw tienda_nube data for ``self.tienda_nube_id`` """
        return self.backend_adapter.read(self.tienda_nube_id)
    ...
    def _import_dependency(self, tienda_nube_id, binding_model,
                           importer_class=None, always=False,
                           **kwargs):
        """
        Import a dependency. The importer class is a subclass of
        ``TiendaNubeImporter``. A specific class can be defined.

        :param tienda_nube_id: id of the tienda_nube id to import
        :param binding_model: name of the binding model for the relation
        :type binding_model: str | unicode
        :param importer_cls: :py:class:`openerp.addons.connector.\
                                        connector.ConnectorUnit`
                             class or parent class to use for the export.
                             By default: TiendaNubeImporter
        ...
        """
        if not tienda_nube_id:
            return
        if importer_class is None:
            importer_class = TiendaNubeImporter
        binder = self.binder_for(binding_model)
        if always or not binder.to_odoo(tienda_nube_id):
            importer = self.unit_for(importer_class, model=binding_model)
            importer.run(tienda_nube_id, **kwargs)
    ...
    def _map_data(self):
        ...
        """
        return self.mapper.map_record(self.tienda_nube_record)
    ...
    def _get_binding(self):
        """Return the openerp id from the tienda_nube id"""
        return self.binder.to_odoo(self.tienda_nube_id)
    ...
    def _create(self, data):
        ...
        _logger.debug(
                '%d created from tienda_nube %s', binding, self.tienda_nube_id)
        return binding
    ...
    def _update(self, binding, data):
        ...
        _logger.debug(
                '%d updated from tienda_nube %s', binding, self.tienda_nube_id)
        return

    def _before_import(self):
        """ Hook called before the import, when we have the TiendaNube
        data"""
        return
    ...
    def run(self, tienda_nube_id, **kwargs):
        """ Run the synchronization

        :param tienda_nube_id: identifier of the record on TiendaNube
        """
        self.tienda_nube_id = tienda_nube_id
        ...
        if not self.tienda_nube_record:
            self.tienda_nube_record = self._get_tienda_nube_data()
        ...
        if not binding:
            with self.do_in_new_connector_env() as new_connector_env:
                ...
                if binder.to_odoo(self.tienda_nube_id):
    ...
    def _import(self, binding, **kwargs):
        ...
        self.binder.bind(self.tienda_nube_id, binding)
...
class AddCheckpoint(ConnectorUnit):
    ...
    (not the tienda_nube.* but the _inherits'ed model) """
...
class DirectBatchImporter(BatchImporter):
    """ Import the TiendaNube Shop Groups + Shops
...
class TranslatableRecordImporter(TiendaNubeImporter):
    ...
    def _get_odoo_language(self, tienda_nube_id):
        language_binder = self.binder_for('tienda_nube.res.lang')
        erp_language = language_binder.to_odoo(tienda_nube_id)
        return erp_language
        ...
        for field in _fields:
            for language in record[field]['language']:
                ...
                if not code:
                    ...
                    raise FailedJobError(
                            _('No language could be found for the TiendaNube lang '
    ...
    def _import(self, binding, **kwargs):
        ...
        """
        # split tienda_nube data for every lang
        split_record = self._split_per_language(self.tienda_nube_record)
...
@job(default_channel='root.tienda_nube')
def import_record(
        session, model_name, backend_id, tienda_nube_id, **kwargs):
    """ Import a record from TiendaNube """
    backend = session.env['tienda_nube.backend'].browse(backend_id)
    env = backend.get_environment(model_name, session=session)
