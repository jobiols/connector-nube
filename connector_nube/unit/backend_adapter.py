...
def api_handle_errors(message=''):
    ...
    except TiendaNubeWebServiceError as err:
        raise exceptions.UserError(
...
class TiendaNubeLocation(object):
...
class TiendaNubeCRUDAdapter(CRUDAdapter):
    """ External Records Adapter for TiendaNube """
    ...
    def __init__(self, environment):
        ...
        """
        super(TiendaNubeCRUDAdapter, self).__init__(environment)
        self.tienda_nube = TiendaNubeLocation(
                self.backend_record.location.encode(),
                self.backend_record.webservice_key
        )
        self.client = TiendaNubeWebServiceDict(
                self.tienda_nube.api_url,
...
class GenericAdapter(TiendaNubeCRUDAdapter):

    _model_name = None
    _tienda_nube_model = None
    ...
    def search(self, filters=None):
        ...
        _logger.debug(
                ...
                self._tienda_nube_model, unicode(filters))
        return self.client.search(self._tienda_nube_model, filters)
    ...
    def read(self, id, attributes=None):
        ...
        _logger.debug(
                ...
                self._tienda_nube_model, str(id), unicode(attributes))
        res = self.client.get(self._tienda_nube_model, id, options=attributes)
    ...
    def create(self, attributes=None):
        ...
        _logger.debug(
                ...
                self._tienda_nube_model, unicode(attributes))
        return self.client.add(self._tienda_nube_model, {
    ...
    def write(self, id, attributes=None):
        ...
        _logger.debug(
                ...
                self._tienda_nube_model,
                unicode(attributes)
        )
        return self.client.edit(
                self._tienda_nube_model, {self._export_node_name: attributes})
    ...
    def head(self, id=None):
        """ HEAD """
        return self.client.head(self._tienda_nube_model, resource_id=id)
