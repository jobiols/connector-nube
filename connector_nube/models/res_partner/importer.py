...
class PartnerImportMapper(ImportMapper):
    ...
    def pricelist(self, record):
        binder = self.binder_for('tienda_nube.groups.pricelist')
    ...
    def groups(self, record):
        ...
        if not isinstance(groups, list):
            groups = [groups]
        model_name = 'tienda_nube.res.partner.category'
...
@tienda_nube
class ResPartnerRecordImport(TiendaNubeImporter):
    _model_name = 'tienda_nube.res.partner'

    def _import_dependencies(self):
        groups = self.tienda_nube_record.get('associations', {}) \
            .get('groups', {}).get(
                self.backend_record.get_version_ps_key('group'), [])
        if not isinstance(groups, list):
            groups = [groups]
        for group in groups:
            self._import_dependency(
                    group['id'], 'tienda_nube.res.partner.category')
...
class AddressImportMapper(ImportMapper):
    ...
    direct = [
        ...
        ('date_add', 'date_add'),
        ('date_upd', 'date_upd'),
        (backend_to_m2o('id_customer'), 'tienda_nube_partner_id'),
    ]
    ...
    def parent_id(self, record):
        parent = self.binder_for('tienda_nube.res.partner').to_odoo(
    ...
    def country(self, record):
        if record.get('id_country'):
            binder = self.binder_for('tienda_nube.res.country')
...
@tienda_nube
class AddressImporter(TiendaNubeImporter):
    _model_name = 'tienda_nube.address'
