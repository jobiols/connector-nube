...
@tienda_nube
class TiendaNubeSaleOrderOnChange(SaleOrderOnChange):
...
class SaleImportRule(ConnectorUnit):
    ...
    def _get_paid_amount(self, record):
        payment_adapter = self.unit_for(
                GenericAdapter,
                '__not_exist_tienda_nube.payment'
        )
...
class SaleOrderMapper(ImportMapper):
    ...
    direct = [
        ('date_add', 'date_order'),
        ('invoice_number', 'tienda_nube_invoice_number'),
        ('delivery_number', 'tienda_nube_delivery_number'),
    ...
    def _get_discounts_lines(self, record):
        ...
        adapter = self.unit_for(
                GenericAdapter, 'tienda_nube.sale.order.line.discount')
    ...
    children = [
        (_get_sale_order_lines,
         'tienda_nube_order_line_ids', 'tienda_nube.sale.order.line'),
        (_get_discounts_lines,
         'tienda_nube_discount_line_ids', 'tienda_nube.sale.order.line.discount')
    ]
    ...
    def carrier_id(self, record):
        if record['id_carrier'] == '0':
            return {}
        carrier = self.binder_for('tienda_nube.delivery.carrier').to_odoo(
...
@tienda_nube
class SaleOrderImport(TiendaNubeImporter):
    _model_name = ['tienda_nube.sale.order']
    ...
    def _import_dependencies(self):
        ...
        self._import_dependency(
                record['id_address_invoice'], 'tienda_nube.address'
        )
        self._import_dependency(
                record['id_address_delivery'], 'tienda_nube.address'
        )

        if record['id_carrier'] != '0':
            self._import_dependency(
                    record['id_carrier'], 'tienda_nube.delivery.carrier')
        ...
        for order in orders:
            try:
                self._import_dependency(
                        order['product_id'], 'tienda_nube.product.template')
    ...
    def _check_refunds(self, id_customer, id_order):
        backend_adapter = self.unit_for(
                GenericAdapter, 'tienda_nube.refund'
        )
        ...
        for refund_id in refund_ids:
            ...
            if refund['id_order'] == id_order:
                continue
            self._import_dependency(refund_id, 'tienda_nube.refund')
    ...
    def _has_to_skip(self):
        ...
        return rules.check(self.tienda_nube_record)
...
class SaleOrderLineMapper(ImportMapper):
    ...
    def _find_tax(self, ps_tax_id):
        binder = self.binder_for('tienda_nube.account.tax')
...
@tienda_nube
class SaleOrderLineRecordImport(TiendaNubeImporter):
    _model_name = [
        'tienda_nube.sale.order.line',
    ]

    def run(self, tienda_nube_record, order_id):
        """ Run the synchronization

        :param tienda_nube_record: record from TiendaNube sale order
        """
        self.tienda_nube_record = tienda_nube_record
        ...
        map_record = self.mapper.map_record(self.tienda_nube_record)
...
def import_orders_since(session, backend_id, since_date=None):
    """ Prepare the import of orders modified on TiendaNube """
    filters = None
    if since_date:
    ...
    try:
        import_batch(session, 'tienda_nube.mail.message', backend_id, filters)
    except:
        pass
